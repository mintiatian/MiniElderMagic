...
export class GamePad {
    constructor(opts = {}) {
        ...
        this._defDown = false;   // ★ 防御ボタン押下フラグ
        this._injectCSS();
    ...
    _buildStaticUI() {
        ...
        /* DEFENSE (mouse right) ─ 攻撃ボタンの真上に配置 */
        const def = this._btn('🛡', ['h']);
        Object.assign(def.style, {
            right: `${16 * this.scale}px`,
            bottom: `${16 * this.scale + base * 1.5 + 12 * this.scale}px`,  // atk の上 + 余白
            width: `${base * 1.2}px`,
            height: `${base * 1.2}px`,
            fontSize: `${base * 1.2 * 0.4}px`,
        });
        /* ─ 防御 pointer：H キー押下／離しを合成 ─ */
        def.addEventListener('pointerdown', e => {
            if (this._defDown) return;
            e.preventDefault();
            e.stopPropagation();
            def.setPointerCapture(e.pointerId);
            this._emitKey('h', 'keydown');
            this._defDown = true;
        }, {passive: false});
        ...
        const defEnd = e => {
            if (!this._defDown) return;
            e.preventDefault();
            this._emitKey('h', 'keyup');
            this._defDown = false;
            def.releasePointerCapture(e.pointerId);
        };
        def.addEventListener('pointerup', defEnd, {passive: false});
        def.addEventListener('pointercancel', defEnd, {passive: false});
    ...
    _updateActive(x, y) {
        ...
        if (cur) {
            this._release(cur);
            this._ptrMap.delete(pid);
        }
        if (act) {
            this._press(act);
            this._ptrMap.set(pid, act);
        }
    }
    ...
    /* ==== Synthetic Key Event ==== */
    _emitKey(k, type) {
        const code = {w: 'KeyW', a: 'KeyA', s: 'KeyS', d: 'KeyD', Tab: 'Tab', h: 'KeyH'};  // ★追加
        const kc = {w: 87, a: 65, s: 83, d: 68, Tab: 9, h: 72};      // ★追加
    ...
    _log(m) {
        if (this.debug) console.log('[GamePad]', m);
    }
}
